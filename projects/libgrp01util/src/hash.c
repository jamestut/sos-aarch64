...
inline uint64_t MurmurHash64A ( const void * key, size_t len, unsigned int seed );

uint64_t hash(const void* data, size_t len)
{
    // just use a pregenerated random number as seed
    return MurmurHash64A(data, len, 359582838);
}
